...
try:
    def get_arguments():
        ...
        if not data.quote and not data.file:
            ...
            if os.path.exists(data.file) == False:
                print(
                    "[!] Quotemaker Error:005\n(code to check in readme.md)\n[-] Specified File {} doesn't exists Please Check and try again".format(
                        data.file))
                exit()
            if data.separator:
    ...
    def get_temp(num, quote, author, sz, wd, n, logo, f_num="1.otf"):
        back_img = Image.open("template/" + str(num) + ".png")
        ...
        font = ImageFont.truetype("font/font" + str(f_num), size=sz)
        ...
        if num == 1:
            ...
            crd_location = (int(back_img.width * .37) - text_width1 / 2,
                            back_img.height - int(back_img.height * .1) - text_height1 / 2 - 18)
        ...
        NEW.save("Quotes.jpg", quality=100)

       # NEW.save("Quotes/" + str(n) + ".jpg", quality=100)


    def temp1(quote, author, sz, wd, n, logo):
        get_temp(1, quote, author, sz, wd, n, logo, "1.otf")


    def temp2(quote, author, sz, wd, n, logo):
        get_temp(2, quote, author, sz, wd, n, logo, "3.otf")
        ...
        if quote_data:
            if quote_data["type"] == 1:
                if quote_data["template"] == None:
                    ...
                    print(
                        "[+] Image has been successfully saved in Quotes.jpg\n [#] You can request more template in our github page".format(
                            file_name))
                else:
                    file_name = random_dat(1)
                    quotesmaker(quote_data["quote"], quote_data["author"], file_name, quote_data["template"],
                                quote_data["logo"])
                    print(
                        "[+] Image has been successfully saved in Quotes.jpg\n [#] You can request more template in our github page".format(
                            file_name))
                ...
                if quote_data["template"] == None:
                    for n in range(len(quotes_file)):
                        t_num = random_dat(0)
                        t = n + 1
                        quotesmaker(quotes_file[n].split(quote_data["separator"])[0],
                                    quotes_file[n].split(quote_data["separator"])[1], t, t_num, quote_data["logo"])
                    print(
                        "[+] Image has been successfully saved in Quotes/1-{}.jpg\n [#] You can request more template in our github page".format(
                            t))
                else:
                    for n in range(len(quotes_file)):
                        t = n + 1
                        quotesmaker(quotes_file[n].split(quote_data["separator"])[0],
                                    quotes_file[n].split(quote_data["separator"])[1], t, quote_data["template"],
                                    quote_data["logo"])
                    print(
                        "[+] Image has been successfully saved in Quotes/1-{}.jpg\n [#] You can request more template in our github page".format(
                            t))
